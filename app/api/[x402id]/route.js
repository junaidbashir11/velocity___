...
import {X402Proxy_GET} from "../../utils/proxies"
...
export async function GET(req,{params}) {
  ...
  const paymentRequirements = await x402.createPaymentRequirements({
    ...
    config: {
      ...
      resource:  `https://xvelocity.dev/api/${id}`                  //`http://localhost:3000/api/${id}`,
    }
  });
  ...
  await UpdateMarketData("GET",id,200)
  const data={
  ...
  const proxy_response=await X402Proxy_GET(req,found_endpoint)
